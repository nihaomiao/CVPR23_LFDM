...
class ReconstructionModel(torch.nn.Module):
    ...
    def forward(self, x):
        ...
        if self.loss_weights['photometric'] != 0:
            # nhm: this part seems to be useless
            # occlusion_map = F.interpolate(generated['occlusion_map'], size=x['driving'].shape[2:], mode='bilinear')
            # value = (torch.abs(generated['deformed'] - x['driving']) * occlusion_map).mean()
            value = torch.abs(generated['deformed'] - x['driving']).mean()
            loss_values['photometric'] = value * self.loss_weights['photometric']
        else:
            loss_values['photometric'] = torch.tensor([0.0]).cuda()

        return loss_values, generated


# nhm added cycle training
class CrossReconstructionModel(torch.nn.Module):
    """
    Merge all updates into single model for better multi-gpu usage
    """

    def __init__(self, region_predictor, bg_predictor, generator, train_params):
        super(CrossReconstructionModel, self).__init__()
        self.region_predictor = region_predictor
        self.bg_predictor = bg_predictor
        self.generator = generator
        self.train_params = train_params
    ...
    def forward(self, x, use_cross=True):
        if not use_cross:
            x['cross'] = x['driving'].clone().detach()
        source_region_params = self.region_predictor(x['source'])
        cross_region_params = self.region_predictor(x['cross'])
        driving_region_params = self.region_predictor(x['driving'])

        cross_bg_params = self.bg_predictor(x['source'], x['cross'])
        cross_generated = self.generator(x['source'], source_region_params=source_region_params,
                                         driving_region_params=cross_region_params, bg_params=cross_bg_params)
        cross_generated.update({'source_region_params': source_region_params, 'driving_region_params': cross_region_params})

        x_inter = cross_generated['prediction']
        inter_region_params = self.region_predictor(x_inter)

        bg_params = self.bg_predictor(x_inter, x['driving'])
        generated = self.generator(x_inter, source_region_params=inter_region_params,
                                   driving_region_params=driving_region_params, bg_params=bg_params)
        generated.update({'source_region_params': inter_region_params, 'driving_region_params': driving_region_params})
        ...
        return loss_values, generated, cross_generated, x_inter


# nhm added for joint training all modules
class JointModel(torch.nn.Module):
    """
    Merge all updates into single model for better multi-gpu usage
    """

    def __init__(self, region_predictor, bg_predictor, generator, avd_network, train_params,
                 train_avd=False):
        super(JointModel, self).__init__()
        self.train_avd = train_avd
        self.region_predictor = region_predictor
        self.bg_predictor = bg_predictor
        self.generator = generator
    ...
    def forward(self, x):
        ...
        if self.train_avd:
            new_region_params = self.avd_network(source_region_params, driving_region_params)
        else:
            with torch.no_grad():
        ...
        generated = self.generator(x['source'], source_region_params=source_region_params,
                                   driving_region_params=new_region_params, bg_params=bg_params)
        generated.update({'source_region_params': source_region_params, 'driving_region_params': new_region_params})
